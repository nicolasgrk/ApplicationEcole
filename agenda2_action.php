...
if (isset ($_GET['action']))
{
	if ($_GET['action'] == 'modifier')//Si l'action est égale à modifier alors on fait:
	{
		...
		<html>
		<head>
			<meta http-equiv="refresh" content="0 ; url=agenda2.php"><!-- Rafraichissement/retour de la page joueur-->
		</head>
		<body>
		</body>
		</html>
	...
	if ($_GET['action'] == 'ajouter')//Si l'action est égale à ajouter alors on fait:
	{
		...
		$req_pre->bindValue(':id_utilisateur',$_POST['formateur'] , PDO::PARAM_INT);
        $req_pre->execute();
